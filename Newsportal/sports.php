...
<?php
  ...
  $query = mysql_query("SELECT * FROM headline ORDER BY id desc");

  while($row=mysql_fetch_array($query)){

    $headline = $row['headline_title'];
    $content = substr($row['headline_content'],0,300);
    $image = $row['headline_image'];
    $post_id= $row['id'];
    ...
    <a href="sportsfull.php?p_id=<?php echo "$post_id"?>"id="Read">Read More...</a>
